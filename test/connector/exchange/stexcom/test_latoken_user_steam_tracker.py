...
from hummingbot.connector.exchange.stexcom.stexcom_user_stream_tracker import StexcomUserStreamTracker
...
class StexcomUserStreamTrackerUnitTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        ts = TimeSynchronizer()
        auth = StexcomAuth(conf.stexcom_api_key, conf.stexcom_secret_key, ts)
        cls.trading_pair = ["ETH-USDT"]
        throttler = AsyncThrottler(CONSTANTS.RATE_LIMITS)
        api_factory = StexcomWebAssistantsFactory(auth=auth)
        data_source = StexcomAPIUserStreamDataSource(
            auth=auth, domain=domain, api_factory=api_factory, throttler=throttler)
        cls.user_stream_tracker: StexcomUserStreamTracker = StexcomUserStreamTracker(
