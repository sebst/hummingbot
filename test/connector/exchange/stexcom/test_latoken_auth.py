...
class TestAuth(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        api_key = conf.stexcom_api_key
        secret_key = conf.stexcom_secret_key
        cls.trading_pair = "BTC-USDT"
        ts = TimeSynchronizer()
        auth = StexcomAuth(api_key, secret_key, ts)
        cls.api_factory = StexcomWebAssistantsFactory(auth=auth)
    ...
    async def ws_auth(self) -> Dict[Any, Any]:
        ...
        listen_key = (await self.rest_auth(CONSTANTS.USER_ID_PATH_URL))["id"]  # this is the getUserId from the github stexcom api client library
